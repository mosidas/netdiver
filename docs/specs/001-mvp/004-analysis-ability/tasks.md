...
- [ ] 6. ドキュメント反映と目視での確認
  ...
  - [x] 6.3 `analysis_overwrite_dev_stage.tscn` を起動して上書きを目視で確かめる
    _Requirements: 11.7_
    _Boundary: AnalysisDevStage_
...
- **タスク 6.3 の目視の記録(要件 11.7)**:

  **いつ・どの環境で**: 2026-08-19。タスク 6.2 と同じ環境(macOS Darwin 25.5.0 / Apple M2、Godot `4.7.1.stable.official.a13da4feb`、Metal 4.0 Forward+ の GUI、物理 60 Hz)。起動は `docs/testing.md` に追記した 2 つ目のコードブロック(`godot --path <プロジェクトのルート> res://src/stage/analysis_overwrite_dev_stage.tscn`)。確認の方法と限界は 6.2 の記録と同じ(複製側の計測シーンから `Input.parse_input_event()`、作業ツリーは無変更)。

  **確認できたこと**:

  - **エラーは 0 件である。** 6.27 秒(376 物理フレーム)の一巡で、出力は Godot のバナーだけだった。
  - **配置は spec.md §8 のとおり。** PNG で `Player`(水色)・`ShooterEnemy`(青)・`ShooterEnemy2`(青)の 3 体が同じ床の上に並び、突進型が居ないことを目で確認した。地形は 1 つ目の仮ステージと同じ見た目である。
  - **要件 11.7(同種別の再取得は上書きであり加算ではない)。数値で確かめた。**
    - 1 体目の `ShooterEnemy` を frame 85 で撃破 → 演出が frame 86 に出て frame 110 に到達 → `remaining_uses` 0 → **3**。
    - frame 143 に拡散弾を 1 回撃って `remaining_uses` 3 → **2** へ減らした。
    - 2 体目の `ShooterEnemy2` を frame 181 で撃破。**撃破の直前の残り回数は 2。**
    - 演出が frame 182 に (248.0, 84.0) から出て frame 206 に到達(飛翔は 24 フレーム = 0.400 秒)。**到達の直後の残り回数は 3。**
    - **加算なら 5(2 + 3)になるはずのところ 3 に戻っており、`ability_uses` による置き換えであることが実機で確認できた。**
  - **上書きの直後からまた撃てる。** 上書き後の frame 245 に拡散弾が出て `remaining_uses` は 3 → 2 になった(要件 7.3 の「取得の直後はクールダウンが明けている」が再取得でも効いている)。
  - **偶然の観測(記録として残す)**: 2 体目を撃破したのは主武器ではなく、**frame 143 に撃った拡散弾の中央の弾**だった。`ability_damage` 20 に対し `ShooterEnemy` の `max_hp` は 20 であり、200px 先の 2 体目を 1 発で撃破している。台本では別のタイミングで主武器で倒す予定だったが、結果として「**拡散弾を撃つと、その流れ弾が次の射撃型を倒して能力が上書きされる**」という連鎖が実機で起きた。上書きの確認としてはむしろ良い条件(減らしてから上書きする順)になっている。この連鎖が手触りとして面白いかはユーザーの判断に属する。
