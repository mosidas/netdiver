...
撃破からの解析(断片の出現・取得・強化した主武器の手触り)は、床・壁とプレイヤー・射撃型 1 体・突進型 1 体を置いた仮ステージ `src/stage/analysis_dev_stage.tscn` を直接起動して目で確かめる。**解析の確認用の仮ステージはこの 1 つだけである**。確かめることはすべてこのシーンに収まる。

```sh
godot --path <プロジェクトのルート> res://src/stage/analysis_dev_stage.tscn
```

このステージで確かめるのは次の 7 つである。

- 射撃型を撃破すると、その撃破位置に断片が現れる
- 突進型を撃破しても、断片は現れない
- 断片に触れると断片が消え、主武器の弾が 3 発の拡散になる(広がりが 8 方向のどの向きでも見て取れるかもあわせて記録する)
- 強化中に副武器を撃つと、弾の色が強化前と変わる
- 断片を 10 秒以上放置しても、同じ位置に現れたままである(時間で消えず、落ちない)
- 強化を得た状態でプレイヤーが死亡してシーンが再読込されると、主武器の弾が 1 発へ戻る
- 撃破から取得を経て、拡散で次の敵を倒すまでを通しで操作しても、操作が止まらない(所要時間と、強化が報酬に感じられたかもあわせて記録する)

どの仮ステージにも共通して、次のことに注意する。
